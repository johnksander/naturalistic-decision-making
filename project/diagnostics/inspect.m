...
%This code plots data saved from simulations with diag_model(). 

%You can either 
%1) specify the job name & ID number here to run this code by itself 
%   (i.e. load and plot some saved results). 
%2) Alternately, you can retrieve that information from bash environment variables 
%   with getenv(). This allows you to call inspect.m immediately after
%   running a diag_model() simulation. See driver_example.m for an example
%   workflow like this. 

sname = getenv('SIM_NAME'); %sname = 'mixstim_test';
jobID = str2num(getenv('JID')); %jobID = 2;

addpath('../') %set_options() lives in the main project directory 

%---setup---------------------
options = set_options('modeltype','diagnostics',...
    'comp_location','local',...
    'sim_name',sname,... %job name you want to plot 
    'jobID',jobID); %job ID you want to plot 
