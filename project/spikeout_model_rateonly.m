function modelfile = spikeout_model_rateonly(options)
...
[EtoE,EtoI,ItoE,...
    ItoI,ItoI_cross,EtoE_cross] = connection_logicals(celltype,pool_options.num_cells);
...
switch options.conn_scheme
    ...
    case 'reviews' 
        % adding a few connections, see circuit diagram for info 
        connection_scheme = EtoE | EtoI | ItoE | ItoI | ItoI_cross | EtoE_cross;
end
...
switch options.conn_scheme
    case 'reviews'
        W(W > 0 & ItoI) = options.ItoI;
        W(W > 0 & ItoI_cross) = options.ItoI_cross;
        W(W > 0 & EtoE_cross) = options.EtoE_cross;
end
